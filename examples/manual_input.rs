...
fn main() {
	...
	while implementation.is_none() {
		...
		println!("2 - Mediantor as a sorted list");
		...
		implementation = match implementation_idx {
			0 => Some(MediantorImplementation::MediantorHeap),
			1 => Some(MediantorImplementation::MediantorSqrtDecomp),
			2 => Some(MediantorImplementation::MediantorSortedVec),
			_ => None,
		}
	}
